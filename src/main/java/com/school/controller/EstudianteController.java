...
//	@PreAuthorize("hasRole('ADMIN')")
	/*
	@GetMapping("/buscarGrado")
	public ResponseEntity<?> getEstudiantePorUserAndPassword2(@RequestParam String username, @RequestParam String password){

		Estudiante estudiante = null;
		Map<String, Object> response = new HashMap<>();

		try {
			estudiante = estudianteService.findByGradoAndGrado(username, password);
		} catch (DataAccessException e) {
			response.put("mensaje", "Error al consultar el estudiante en la base de datos");
			response.put("error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
		...
		if(estudiante == null) {
			response.put("mensaje", "El estudiante con el Grado: ".concat(username.toString().concat(" no existe en la base de datos")));
	...
	@PreAuthorize("hasAnyRole('ADMIN', 'ESTUDIANTE')")
	@GetMapping("/porGrado")
	public ResponseEntity<?> getEstudiantePorGrado(@RequestParam("grado") String dni){

		Estudiante estudiante = null;
		Map<String, Object> response = new HashMap<>();

		try {
			estudiante = estudianteService.findByGrado(estudiante.getGrado());
		} catch (DataAccessException e) {
			response.put("mensaje", "Error al consultar el estudiante en la base de datos");
			response.put("error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
